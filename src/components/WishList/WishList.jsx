...
const WishList = ({ wishList, handleDeleteWishList }) => {
  ...
  const { product_image, product_title, price, description, product_id } = wishList;
  return (
    <div className="flex justify-stretch gap-4 p-4 border rounded-xl">
      ...
      <div className="flex items-center w-[100%] justify-between">
        <div className="flex flex-col gap-2">
          ...
          <button className="btn" onClick={() => handleMarkAsWishList(product_id)}>Add to Card</button>
        </div>
        <button className="btn" onClick={() => handleDeleteWishList(product_id)}>
          Delete
        </button>
      </div>
    </div>
